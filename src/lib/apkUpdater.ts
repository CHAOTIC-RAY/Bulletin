 ...
 * Checks the latest release, downloads the APK, and hands off to the system
 * installer via the native ApkInstall Capacitor plugin. On the web build the
 * native plugin is a no-op, so callers fall back to a download link.
 */
...
const GITHUB_REPO = "CHAOTIC-RAY/Bulletin";
const AUTO_UPDATE_KEY = "bulletin_apk_auto_update";
const SNOOZE_KEY = "bulletin_apk_update_snooze_until";
const LAST_CHECK_KEY = "bulletin_apk_last_check_at";
const LAST_KNOWN_KEY = "bulletin_apk_last_known_remote";
...
type ApkInstallPluginApi = {
  ...
  downloadAndInstall(options: { url: string; fileName?: string }): Promise<{ ok?: boolean }>;
  addListener(
...
// No-op on web; only resolves inside the APK where the native plugin exists.
...
function pickApkAsset(assets: Array<Record<string, unknown>> | undefined): {
  ...
  return {
    ...
    name: String(apk.name || "Bulletin.apk"),
...
/** Check GitHub for a newer APK than the installed one. */
...
export async function downloadAndInstallApk(
  ...
  if (!allowed) {
    ...
    throw new Error("Allow Bulletin to install updates, then try again.");
  }

  onProgress?.({ percent: 0 });

  const handle = await ApkInstall.addListener(
    "apkDownloadProgress",
    (event: { percent: number; bytes?: number; total?: number }) => {
    onProgress?.({ percent: event.percent ?? 0, bytes: event.bytes, total: event.total });
  });

  try {
    await ApkInstall.downloadAndInstall({
      url: release.apkUrl,
      fileName: release.apkName || `Bulletin-${release.versionName}.apk`,
    });
...
export async function maybeAutoCheckApkUpdate(options?: {
  ...
  try {
    ...
    if (result.update && options?.notify !== false) {
      ...
      window.dispatchEvent(new CustomEvent("bulletin-apk-update", { detail: result.update }));
    }
...
/** Web/desktop-friendly: fetch the latest release APK URL without the installer. */
