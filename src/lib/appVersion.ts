/** Build id + semantic version baked in at compile time (from package.json). */
export const APP_BUILD_ID: string =
  typeof __BULLETIN_BUILD_ID__ !== "undefined" && __BULLETIN_BUILD_ID__
    ? __BULLETIN_BUILD_ID__
    : "dev";

export const APP_VERSION: string =
  typeof __BULLETIN_VERSION__ !== "undefined" && __BULLETIN_VERSION__
    ? __BULLETIN_VERSION__
    : "0.0.0";
 ...
 * (package.json vs version.json) so a redeploy with identical code never
 * triggers a false "update available" prompt. Falls back to buildId only when
 * the version field is missing on one side.
 */
